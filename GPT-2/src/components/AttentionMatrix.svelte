<script lang="ts">
	...
	const expandAttention = () => {
		...
		expandTl
			...
			.from(
				attentionQK.querySelectorAll('svg rect'),
				{
					scale: 0,
	...
	$: qkColorScale = (d, i) => {
		return d3
			...
			.range(['white', theme.colors['red'][700]])(d);
	};
	const maskedColorScale = (d, i) => {
		return d3.scaleLinear().domain([-3, 3]).range(['white', theme.colors['red'][700]])(d);
	};
	const softmaxColorScale = (d, i) => {
		return d3.interpolate('white', theme.colors['red'][700])(d);
	};
</script>
...
<style lang="scss">
	...
	.color-scale {
		...
		.bar {
			...
			background: linear-gradient(90deg, white 0%, theme('colors.red.700') 100%);
		}
		.val {
			flex-shrink: 0;
